...
With the tests done to [Garbodor during November 1st](/tests/gmax_t6/20251101_garbodor.md) we obtained the exact stats of thos boss. However, when testing the following run these stats did not accuratelly represent the observed segment drops... almost. We are facing the bug known as *Max Dupe*, where max moves sometimes get duplicated as the player uses two max moves on the same turn.
...
Taking this into account, the damage stays correct until the 8th cycle. At that point everything becomes chaotic with barely any segment drop fits the expected damage, but with a nice coincidence: If we were to add exactly the damage of 2 max moves, this issue gets perfectly fixed. Doing so the damage stays correct until nearly the end. So, if you think about this, during max phase 1 the bug happened during the 1st turn, but if this happens during the last turn, you will actually get the duplicate moves while not getting removed from other turns, meaning you could get extra damage for the whole run. This is what we think that happened during max phase 7, it feels its not a coincidence at all. Finally, one additional max move during the max phase 24 would fix the remaining issues at the end of the run resulting in perfect calculations at the end.

This may sound as a crazy workarround and yes it shocked us in the research thing, but take this into account:
- The glitch is perfectly observed during max phase 1.
- The damage calculations per move are right, as if we consider we made a mistake before cycle 8, the calculated damage from the segment #36 to #82 is exactly 46008, the expected value. The same happens for #36 to #41, #46, #51, #55 and #68.
- We actually didnt made a mistake counting the max moves, as its a extremely simple task with little margin of error and multiple members of the team counted and double checked 2 or 3 times the amount of moves over the whole run.
